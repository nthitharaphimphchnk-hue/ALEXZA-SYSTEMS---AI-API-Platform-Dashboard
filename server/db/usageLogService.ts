...
function warnMissingMongoOnce() {
  if (missingMongoWarned) return;
  missingMongoWarned = true;
  console.error(
    "[UsageLogService] MongoDB is not configured. Set MONGODB_URI in .env to enable usage analytics."
  );
}
...
export async function getUsageStats(projectId: number, hours: number) {
  ...
  try {
    ...
    console.error("[UsageLogService] Failed to load stats", e);
    return { totalRequests: 0, successRate: 0, avgResponseTime: 0, totalCost: 0 };
  }
}
...
export async function getUsageByHour(projectId: number, hours: number) {
  ...
  try {
    ...
    console.error("[UsageLogService] Failed to load hourly usage", e);
    return [];
  }
}
...
export async function getRecentUsageLogs(projectId: number, limit: number = 20) {
  if (!isMongoConfigured()) {
    warnMissingMongoOnce();
    return [];
  }
  try {
    ...
    return rows.map((row) => ({
      ...
      endpoint: row.endpoint ?? "POST /tti/decide-font",
      statusCode: row.statusCode ?? (row.success ? 200 : 500),
      responseTimeMs: row.responseTimeMs ?? 0,
      credits: row.creditsUsed ?? 1,
    }));
  } catch (e) {
    console.error("[UsageLogService] Failed to load recent usage", e);
    return [];
  }
}
